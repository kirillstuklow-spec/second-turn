...
func _test_decay_kill_ends_battle_before_next_activation() -> void:
	...
	assert(combat_event_log.history[-3].kind == CombatEvent.Kind.DAMAGE_APPLIED)
	assert(combat_event_log.history[-3].source_type == &"decay")
	assert(combat_event_log.history[-3].target_unit == target)
	assert(combat_event_log.history[-2].kind == CombatEvent.Kind.HEALTH_LOST)
